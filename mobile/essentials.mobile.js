...
async function captureReport() { 
    ...
    const flow = await lighthouse.startFlow(page, { //define a tool that will measure UI
        name: 'essentials', //set any name
        configContext: {
            settingsOverrides: {
                throttling: {
                    rttMs: 150, // recommendation for Google- Ok
                    throughputKbps: 1638.4, //~ 10Mb Internet, recommendation for Google, don't write more than possible
                    cpuSlowdownMultiplier: 4, // 1 - use full CPU, 2 - use half of CPU
                ...
                screenEmulation: { // not to change
                    mobile: true, // true if we emulate a mobile view
                    width: 390, // change if we emulate a mobile view
                    height: 844, // change if we emulate a mobile view
                    deviceScaleFactor: 3, // not to change
                    disabled: false, // not to change
                },
                formFactor: "mobile", //emulation of the desktop/mobile view of Chrome
