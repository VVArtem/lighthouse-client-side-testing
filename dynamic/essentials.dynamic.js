...
async function captureReport() { //we will call the function at the end of the script
    ...
    const flow = await lighthouse.startFlow(page, { //define a tool that will measure UI
        name: 'essentials-dynamic', //set any name
        configContext: {
            settingsOverrides: {
                throttling: {
    ...
    let TablesPLP = categoryUrls[Math.floor(Math.random() * categoryUrls.length)];
    ...
    //CSS Selectors
    const productSelector = ".product-list .priced a";
    ...
    const productLinks = await page.$$eval(productSelector, links => links.map(link => link.href));
    if (productLinks.length === 0) {
        throw new Error(`Not found any products in ${TablesPLP}`);
    }
    const randomProductLink = productLinks[Math.floor(Math.random() * productLinks.length)];


    await flow.navigate(randomProductLink, {
